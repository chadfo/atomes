...
(() => {
  ...
  // ── Per-slot sidecar files ────────────────────────────────────────────
  // The single combined JSON hits the host's per-write size cap once a few
  // photos are stored (writes past ~2MB fail silently). Each slot therefore
  // persists in its own .imgslot-<id>.state.json; the combined file is kept
  // read-only as a legacy fallback for pre-shard drops.
  const fileFor = (id) => 'imgslot-' + String(id).replace(/[^a-zA-Z0-9_-]/g, '_') + '.state.json';
  const idFetched = new Set();   // per-id file fetch already started
  const authoritative = new Set(); // ids changed in-session; disk must not clobber
  function loadId(id) {
    if (!id || idFetched.has(id)) return;
    idFetched.add(id);
    fetch(fileFor(id) + '?v=' + Date.now(), { cache: 'no-store' })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => {
        if (j && j.u && !authoritative.has(id) && !tombstones.has(id)) {
          slots[id] = j;
          subs.forEach((fn) => fn());
        }
      })
  ...
  function saveId(id) {
    ...
    if (!w || !id) return;
    const body = JSON.stringify(slots[id] || {});
    saveQ[id] = (saveQ[id] || Promise.resolve())
      .then(() => w(fileFor(id), body))
      .catch(() => {});
  }

  function load() {
    if (loadP) return loadP;
    loadP = fetch(STATE_FILE + '?v=' + Date.now(), { cache: 'no-store' })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => {
  ...
  function setSlot(id, val) {
    ...
    subs.forEach((fn) => fn());
    // A drop is rare + high-value — write immediately (own file per slot) so
    // nav-away can't lose it and other slots' writes can't clobber it.
  ...
  const stylesheet =
    ...
    '  font:13px/1.3 system-ui,-apple-system,sans-serif;width:240px;height:160px}' +
    '.frame{position:absolute;inset:0;overflow:hidden;background:color-mix(in srgb,currentColor 8%,transparent)}' +
    ...
    '.empty .sub u{text-underline-offset:2px;text-decoration-color:color-mix(in srgb,currentColor 30%,transparent)}' +
    '.empty:hover .sub u{opacity:.85;text-decoration-color:currentColor}' +
    ':host([data-over]) .frame{outline:2px solid #c96442;outline-offset:-2px;' +
    '  background:rgba(201,100,66,.10)}' +
    '.ring{position:absolute;inset:0;pointer-events:none;border:1.5px dashed color-mix(in srgb,currentColor 30%,transparent);' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    attributeChangedCallback(name) {
      // The carousel reuses one element and swaps its id — fetch that slot's
      // sidecar on first sight or the saved image never loads.
      if (name === 'id') loadId(this.id);
